...
class RazorpaySettings(Document):
    ...
    def authorize_payment(self):
        ...
        if self.flags.status_changed_to in ("Authorized", "Verified", "Completed"):
            ...
            task = frappe.get_last_doc('Booking Desk Web')


            frappe.get_doc('Booking Desk Web',
                            task.name).run_method("on_payment_authorized", self.flags.status_changed_to,task.name)
            redirect_url = 'payment-success?doctype={0}&docname={1}&{2}'.format(self.data.reference_doctype, self.data.reference_docname,self.flags.status_changed_to)
            print("custom redirect 1")
            # redirect_url = 'payment-success?doctype={0}&docname={1}'.format(self.data.reference_doctype, self.data.reference_docname)
        else:
            redirect_url = 'payment-failed'

        if redirect_to:
            print("custom redirect 2")
            self.data.reference_doctype
            redirect_url += '&' + urlencode({'redirect_to': redirect_to})
        if redirect_message:
            print("custom redirect 3")
